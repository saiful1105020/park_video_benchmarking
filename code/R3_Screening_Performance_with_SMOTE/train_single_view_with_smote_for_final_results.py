...
class TensorDataset(Dataset):
    ...
    def __init__(self, df, fold="test"):
        ...
        if fold=="train":
            ...
            X_hetero = self.features.tolist()
            y_hetero = self.labels.tolist()
            X_resampled, y_resampled = ros.fit_resample(X_hetero, y_hetero)
            self.features = np.array(X_resampled)
            self.labels = np.array(y_resampled)
...
def main(**cfg):
    ...
    model = SingleViewLinearProbe(n_features=n_features, hidden_dim=all_configs["hidden_dim"], drop_prob=all_configs["drop_prob"])
    model.to(device)
...
def unit_test():
    for task in valid_tasks:
        # for model_name in list(model_embedding_paths.keys())[0:1]:
