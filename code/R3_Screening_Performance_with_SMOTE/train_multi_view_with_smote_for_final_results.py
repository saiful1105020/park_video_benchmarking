...
class TensorDataset(Dataset):
    ...
    '''
    def __init__(self, df, fold="test"):
        ...
        if fold=="train":
            ...
            n_views, n_dim = np.asarray(self.features.tolist()[0]).shape
            # print(np.array(self.features.tolist()[0]).shape) #(4, 768)
            # print(f"#views: {n_views}, #Dim: {n_dim}")

            # Flatten multi-view as single-view
            X_hetero = [np.asarray(x).reshape(-1) for x in self.features.tolist()]
            y_hetero = self.labels.tolist()
            # Resample
            X_resampled, y_resampled = ros.fit_resample(X_hetero, y_hetero)
            # Revert to multi-view shape
            X_resampled = [np.asarray(x).reshape(n_views, n_dim) for x in X_resampled]

            self.features = np.asarray(X_resampled)
            self.labels = np.asarray(y_resampled)
            self.ids = np.zeros(self.labels.shape)
            print(f"Total train samples: {len(self.labels)}, positive: {np.sum(self.labels==1)}, negative: {np.sum(self.labels==0)}")
...
class TensorDatasetLR(Dataset):
    ...
    def __init__(self, df, fold="test"):
        ...
        if fold=="train":
            ...
            # we cannot track unique ids after oversampling, however, it is not needed for training
            self.ids = np.zeros(self.labels.shape) 
            print(f"Total train samples: {len(self.labels)}, positive: {np.sum(self.labels==1)}, negative: {np.sum(self.labels==0)}")
            print(f"Features shape: {self.features.shape}")
        else:
...
@click.command()
@click.option("--task_name", default="finger_tapping", help="Options: see task lists")
...
def main(**cfg):
    ...
    if cfg["task_name"] in ["finger_tapping", "flip_palm", "nose_touch", "open_fist"]:
        train_dataset = TensorDatasetLR(df=dataset_df["train"], fold="train")
        dev_dataset = TensorDatasetLR(df=dataset_df["dev"], fold="dev")
        test_dataset = TensorDatasetLR(df=dataset_df["test"], fold="test")
    else:
        train_dataset = TensorDataset(df=dataset_df["train"], fold="train")
        dev_dataset = TensorDataset(df=dataset_df["dev"], fold="dev")
        test_dataset = TensorDataset(df=dataset_df["test"], fold="test")
