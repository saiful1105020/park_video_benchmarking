...
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))           # .../VFMs/VideoMAEv2
VFMS_ROOT = os.path.abspath(os.path.join(SCRIPT_DIR, ".."))      # .../VFMs
...
MODEL_TAG = "VideoMAEv2"

HF_TOKEN = os.environ.get("HF_TOKEN", None)

# Default HF cache to a repo-local folder: VFMs/.cache/hf
...
def load_processor(model_name: str) -> AutoImageProcessor:
    kwargs = dict(cache_dir=HF_CACHE_DIR)
    if HF_TOKEN:
        kwargs["token"] = HF_TOKEN

    try:
        return AutoImageProcessor.from_pretrained(model_name, use_fast=False, **kwargs)
    except TypeError:
        return AutoImageProcessor.from_pretrained(model_name, **kwargs)


def load_videomaev2_model(model_name: str) -> torch.nn.Module:
    model_kwargs = dict(
...
def main(video_root: str, out_root: str, save_every: int, stride: int, num_views: int):
    ...
    count_filename = os.path.join(
        model_dir,
        f"{MODEL_TAG}_{num_views}views_{stride}stride_Features_All_PARK_Videos_Count_Completed_test.txt"
    )
    pkl_filename = os.path.join(
        model_dir,
        f"{MODEL_TAG}_{num_views}views_{stride}stride_Features_All_PARK_Videos_test.pkl"
    )
    ...
    model = load_videomaev2_model(model_name)
    ...
    for filename in tqdm(filenames, desc=f"Extracting with {MODEL_TAG}"):
        ...
        try:
            ...
            for view_idx, indices in enumerate(multi_view_indices):
                video_np = read_video_pyav(container, indices, reduced_height=mc["image_size"])
                ...
                with torch.no_grad():
                    ...
                    inputs["pixel_values"] = inputs["pixel_values"].permute(0, 2, 1, 3, 4).contiguous()

                    outputs = model(**inputs)
                    last_hidden_states = outputs.last_hidden_state if hasattr(outputs, "last_hidden_state") else outputs.unsqueeze(1)

                    mean_pooled = last_hidden_states.mean(dim=-2).reshape(-1)
                    max_pooled = last_hidden_states.amax(dim=-2).reshape(-1)
